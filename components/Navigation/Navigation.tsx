import * as React from "react";
import Link from "next/link";
import {
  CartCounter,
  CArtImage,
  CartImageAnchor,
  LanguageSelect,
  LanguageSelectIconWrapper,
  LanguageSelectWrapper,
  LeftNavItemWrapper,
  Logo,
  LogoWrapper,
  NavContentWrapper,
  NavItem,
  NavItemWrapper,
  NavWrapper,
} from "./Navigation.styles";
import { Currency } from "../../utilities/types";
import { countCartItems } from "../ProductCard/ProductCard.methods";
//@ts-ignore
import IsScrolling from 'react-is-scrolling';

export const CurrencyList: {
    value: string;
  }[] = Object.values(Currency)
    .filter((value) => typeof value === "string")
    .map((value) => ({ value: value as string }));

 function Navigation({toggleCart,cartOpen}:{ toggleCart:any,cartOpen:boolean}) {
  return (
    <NavWrapper fixed={false}>
      <NavContentWrapper>
        <NavItemWrapper>
          <LogoWrapper>
            <Logo src="https://www.luminskin.com/_next/static/images/logo-20c2cb1d9d2bb6d2139d0e5cec3103bd.png" />
          </LogoWrapper>
          <Link href=" /">
            <NavItem>Shop</NavItem>
          </Link>
          <Link href=" /">
            <NavItem>About</NavItem>
          </Link>
          <Link href=" /">
            <NavItem>Support</NavItem>
          </Link>
          <Link href=" /">
            <NavItem>Blog</NavItem>
          </Link>
        </NavItemWrapper>
        <LeftNavItemWrapper>
          <Link href=" /">
            <NavItem>Account</NavItem>
          </Link>
          <CartImageAnchor onClick={() =>toggleCart(!cartOpen)}>
            <CArtImage src="data:image/png;base64," />
            <CartCounter>{countCartItems()}</CartCounter>
          </CartImageAnchor>
          <LanguageSelectWrapper>
            <LanguageSelect>
              {CurrencyList.map((currency, index) => {
                return (
                  <option key={index} value={currency.value}>
                    {currency.value}
                  </option>
                );
              })}
            </LanguageSelect>
            <LanguageSelectIconWrapper>
              <svg
                viewBox="0 0 24 24"
                role="presentation"
                focusable="false"
                aria-hidden="true"
                style={{ width: "1em", height: "1em", color: "currentcolor" }}
              >
                <path
                  fill="currentColor"
                  d="M16.59 8.59L12 13.17 7.41 8.59 6 10l6 6 6-6z"
                ></path>
              </svg>
            </LanguageSelectIconWrapper>
          </LanguageSelectWrapper>
        </LeftNavItemWrapper>
      </NavContentWrapper>
    </NavWrapper>
  );
}

export default Navigation;
